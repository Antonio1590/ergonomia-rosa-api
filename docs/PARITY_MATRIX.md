...
| Historial personal (`getUserHistory`) | OK (`HistoryPage.tsx`, ruta `/historial`) | OK | **Completa en código — bloqueada en producción** | Implementado el 2026-08-25 en `SheetsService.ts`/`HistoryPage.tsx`. El despliegue real de Apps Script está desactualizado respecto al repo (confirmado en vivo: `"Acción desconocida: getUserHistory"`) — no funcionará hasta redesplegar manualmente desde script.google.com. Ver `docs/CHANGELOG.md` |
